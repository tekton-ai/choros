...
publishes the selection to `$CHOROS_HOME_DIR/state/default-*` pointer files. The agent wrappers
...
Choros never touches the credential stores; the provider CLIs own every login end to end.
...
- Files (`CLAUDE.md`, `config.toml`, `AGENTS.md`) are **copied**, and `settings.json` /
  ...
  file symlink.
- Copies and merged keys are recorded in `<profile>/.choros-profile.json`, so anything the user
...
- Per-account and never shared: credentials, `oauthAccount`/`userID` identity and per-project
  state in `.claude.json`, auth-related settings keys, and Choros's own lifecycle hooks
  (written per profile).
...
mirrors this worktree's `plugins/choros` into `~/.claude/skills/choros-dev`
(a renamed copy so it doesn't collide with the installed prod `choros` plugin),
exposing them as `/choros-dev:<skill>`. Re-run after editing a skill, then
`/reload-plugins`. In production the skills ship inside the real `choros`
plugin, so there's no collision — this is dev-only.

`bun scripts/dev-cli.ts` finds the live host manifest under `<worktree>/choros-dev-data/host/*`,
then runs the dev CLI with `CHOROS_HOME_DIR` (the dev data dir),
`CHOROS_ORGANIZATION_ID` (the live host's org), and a placeholder
`CHOROS_API_KEY` (local host commands use the manifest token, not this). Start
`bun run dev:desktop` and open a workspace first, or the wrapper reports no live
host. `CHOROS_ORGANIZATION_ID` is a general CLI override (mirrors
`CHOROS_API_KEY`), not dev-only.

The whole dev stack binds fixed ports off `CHOROS_PORT_BASE=3960`, so only one
