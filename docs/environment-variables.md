...
```bash
gh secret set MY_VAR -R tekton-ai/choros --body "value"
```
...
- `.env.local.example` — fake working value (`fake-r2-access-key-id`,
  `choros-private-dev`). Local should boot with no real credentials.
...
worktree. So the root `.env` (`~/code/choros/.env`) is the source of truth for
