...
> **前置规格已接受，本文待工程评审。** 产品负责人于 2026-09-07 明确选择“接受新版 spec，重写 plan”。本文依据已接受的 [intent](./intent.md) 与 [spec](./spec.md)，整体替代旧计划；这次授权不等于接受新 plan，也不代表已经实施或验证功能。
>
> **源码核对基线：`origin/main` 的 `527ec4ea1`。** 本轮在当前工作区只修改审批文档。实施前重新核对最新 main；若接口或范围发生实质变化，先修订本文并重新接受。保留旧分支未提交实现，不合并旧分支历史；需要移植时，只向基于最新 main 的干净实现分支移植获批功能。
...
- **状态：** `draft`。新版 spec 已获产品负责人接受；本计划尚待工程负责人接受，迁移等高风险项另需技术负责人/架构负责人审查。设计、安全和合规结论不由 agent 代签。
- **本轮交付：** 记录 spec 接受、重写本计划；未实施功能、未发布。人工接受本文后，才进入代码实施阶段；实质偏离须先修订计划并重新接受。
