...
> **整栏手势修订：2026-09-14，已接受。** 负责人明确要求“手势需要绑定在整个左侧的面板上”，在本轮对话中回复“接受”，并已将 frontmatter 的 `status` 改为 `accepted`。本次将 Mac 触控板横向手势的响应区域从 Profile 名称控件扩大到整个 Dashboard 左侧面板，避免用户必须精确对准小控件。既有功能的历史接受记录不变；后续实施须遵守更新后的 `plan.md` 及其独立审批门槛，旧计划的“手势仅绑定此控件”不授权本次实现。

### 本次增量的验收边界

- 在展开侧栏的顶部、项目/工作区列表、底部及空白处，以及收起后的侧栏内，都可以用触控板双指横向滑动切换相邻 Profile；不要求先点击或聚焦名称控件。主内容区、右侧面板及弹出的菜单/对话框不触发此手势。
- 左滑到下一个、右滑到上一个，沿用当前排序与统一选择入口。一次连续手势及其惯性最多切换一个；在名称控件上操作不能因父子重复监听而切换两次。首尾不循环。
- 正常纵向滚动、轻微横向漂移、缩放、列表拖拽和侧栏宽度调整不能误触切换；不取消既有点击、文字选择或窗口拖动行为。已有可横向滚动的子控件优先处理自身滚动。
- 本次扩大的是触控板横向滚动手势范围，不把整个列表的鼠标按下拖动改成 Profile 切换。名称控件既有点击、键盘与触摸/指针切换保留。
- 在真实 Desktop 分别验证列表和工作区路由、展开/收起、列表已滚动及空 Profile；覆盖左右往返、连续惯性、首尾、名称控件不重复触发，以及上述滚动/拖拽/菜单排除。自动化 wheel 输入不冒充真实 Mac 触控板手感验收。
...
**位置：** 在现有 Dashboard 侧栏顶部，窗口控制/导航行下方、“新建工作区”上方。工作区和列表路由中始终可见。2026-09-14 负责人确认手势可用后，要求加入发现提示和当前 Profile 身份反馈，并明确要求“加一下试试”；本轮展示试用不改变侧栏宽度、Profile 排序、归属或导航语义。

```text
窗口控制 / 导航
当前 Profile 名称 ▾             Profile 2/3
‹           双指左右滑动切换           ›
新建工作区
搜索
工作区 / PR / 插件
...
| 侧栏已有纵向滚动、选择、拖放和窗口拖动区域 | 触控板横向手势覆盖整个左侧面板；保留原有纵向滚动、拖放、窗口控制与拖动行为，不扩展到弹出菜单或主内容区 |
...
2. 在整个左侧面板内用触控板左滑到下一个、右滑到上一个；必须以横向运动为主，一次手势及其惯性最多切换一次，首尾不循环。
3. 整栏触控板横向滚轮与名称控件已有触摸/指针滑动使用相同方向和排序；纵向滚动、双指缩放及 `ctrlKey` 缩放输入不能触发切换。整栏不截获鼠标拖动，具体排除与验收见“本次增量的验收边界”。
...
- 当前名称采用现有前景色、14px 半粗体和轻量背景，固定在列表滚动区之外。位置计数单独展示，不复用待关注徽标。
- 多 Profile 时提供可点击的上一项/下一项按钮与双指滑动提示；悬停显示目标名称，完整说明明确手势适用于整个侧栏。首尾对应按钮禁用，单 Profile 不显示导航提示行。
- 切换时仅名称做 140ms、6px 的方向滑入，不平移主工作区。实时开启减少动态效果后，停止或不启动名称动画；tooltip 也遵守该偏好。
- 收起侧栏显示首字母与紧凑位置计数。切换后展示完整名称和位置约 1.2 秒，不移动输入焦点；悬停仍可持续查看。没有新手引导持久化标记或 Profile 配色字段。
...
- **状态：** `accepted`。负责人已于 2026-09-14 接受整栏手势修订，并本人更新规格与实施计划的 frontmatter。整栏手势代码已实现，自动化验证与物理触控板验收限制见 `plan.md` 的本次验证记录，尚未发布。原功能与临时实验规则的接受记录保留在历史中；负责人于 2026-09-08 在手动验证后明确要求移出 Experimental、默认开放。
