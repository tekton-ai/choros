...
用户后续要求明确 macOS 下载文案、增加 GitHub 入口并加入类似参考站的动态效果。此次追加仅调整 Web：下载说明显示 DMG 与 ARM64/x64；页头及首屏提供 GitHub 链接；CSS 实现分层入场、滚动渐入和悬停反馈，SVG 连线/进度动画最多自动播放 4 秒。减少动态效果时停用页面动画，并用同一 SVG 的 `#still` 静态片段确保嵌入图片也停止运动；无需 JavaScript 或桌面端。验收包含实际导航、逐帧动画差异、减少动态后的图像稳定性及窄屏布局。

用户随后反馈滚动到示意图时动画已结束。本次修复取消连线/进度的 4 秒上限，新增同源 `site-motion.js`，仅用 IntersectionObserver、页面可见性和用户开关控制可视区内循环；离屏、后台、手动暂停或系统减少动态效果时切换为静态片段。脚本不可用时默认静态，下载/文档/FAQ 不受影响。该用户要求替代上文“最多 4 秒、无 JavaScript”的限制，不引入第三方脚本、框架或桌面端。
