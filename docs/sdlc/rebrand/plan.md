...
- **`SUPERSET_*` environment variable names** (`SUPERSET_HOME_DIR`, `SUPERSET_AGENT_ID`, `SUPERSET_TERMINAL_ID`, `SUPERSET_HOST_AGENT_HOOK_URL`, `SUPERSET_ENV`, `SUPERSET_HOOK_VERSION`, `SUPERSET_PANE_ID`, `SUPERSET_TAB_ID`, `SUPERSET_WORKSPACE_ID`, `SUPERSET_DEBUG_HOOKS`, `SUPERSET_LEGACY_WORKTREE_BASE_DIR`, `SUPERSET_AGENT_HOOK_PORT`, `SUPERSET_AGENT_HOOK_VERSION`, `SUPERSET_AGENT` CI-detection var, and any others matching `SUPERSET_[A-Z_]+`). These are the **wire protocol** between the parent Superset dev app and the CLI agents installed on the author's machine (Claude Code, Codex, Factory Droid, Cursor, Gemini, Copilot, Kimi, Grok, Vibe, Mastracode, OMP, etc.). They are inlined into hook commands persisted in `~/.claude/settings.json`, `~/.factory/settings.json`, `~/.codex/config.toml`, etc. — files this fork does not own and cannot rewrite atomically with a rebuild. Renaming them breaks (a) the fork's own dev integration (the parent Superset app exports `SUPERSET_HOME_DIR` when opening this fork as a workspace), and (b) every existing hook installation on the author's and any collaborator's machine. Same protocol-boundary reasoning as `.superset/` directory above.
- Same protocol reasoning: `SUPERSET_HOOK_VERSION`, `SUPERSET_ENV`, and all related shell / env constants in `packages/agent-setup/**` and its template files.
