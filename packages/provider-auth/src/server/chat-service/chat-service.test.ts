...
let testChorosHomeDir: string | null = null;
...
describe("ChatService OpenAI auth storage", () => {
	beforeEach(() => {
		...
		testChorosHomeDir = mkdtempSync(join(tmpdir(), "chat-service-test-"));
	...
	afterEach(() => {
		if (testChorosHomeDir) {
			rmSync(testChorosHomeDir, { recursive: true, force: true });
			testChorosHomeDir = null;
		}
		if (originalChorosHomeDir) {
