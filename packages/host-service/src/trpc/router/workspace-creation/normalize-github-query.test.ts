...
const repo = { owner: "superset-sh", name: "choros" };
	...
	describe("non-GitHub URLs (plain text fallback)", () => {
		test("GitHub repo URL (no entity path)", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/superset-sh/choros",
				repo,
				"pull",
			);
			expect(result.isDirectLookup).toBe(false);
		...
		test("GitHub compare URL", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/superset-sh/choros/compare/main...feature",
				repo,
				"pull",
			);
			expect(result.isDirectLookup).toBe(false);
		...
		test("SSH-style URL", () => {
			const result = normalizeGitHubQuery(
				"git@github.com:superset-sh/choros.git",
				repo,
				"pull",
			);
			expect(result.isDirectLookup).toBe(false);
...
describe("normalizeGitHubQuery — PR URLs", () => {
	describe("same repo", () => {
		test("basic URL", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/superset-sh/choros/pull/3130",
				repo,
				"pull",
			);
			expect(result.query).toBe("3130");
		...
		test("/files tab", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/superset-sh/choros/pull/3354/files",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("/changes tab", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/superset-sh/choros/pull/3354/changes",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("/commits tab", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/superset-sh/choros/pull/3354/commits",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("/checks tab", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/superset-sh/choros/pull/3354/checks",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("trailing slash", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/superset-sh/choros/pull/3354/",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("query params", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/superset-sh/choros/pull/3354?diff=unified",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("query params on tab", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/superset-sh/choros/pull/3354/files?diff=split&w=1",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("hash fragment", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/superset-sh/choros/pull/3354#discussion_r123",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("hash fragment on files tab", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/superset-sh/choros/pull/3354/files#diff-abc123",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("www prefix", () => {
			const result = normalizeGitHubQuery(
				"https://www.github.com/superset-sh/choros/pull/3354",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("http (not https)", () => {
			const result = normalizeGitHubQuery(
				"http://github.com/superset-sh/choros/pull/3354",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("case-insensitive owner/repo", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/Choros-SH/Choros/pull/100",
				repo,
				"pull",
			);
	...
	describe("cross-repo mismatch", () => {
		test("different owner", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/other-org/choros/pull/100",
				repo,
				"pull",
			);
...
describe("normalizeGitHubQuery — issue URLs", () => {
	describe("same repo", () => {
		test("basic URL", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/superset-sh/choros/issues/100",
				repo,
				"issue",
			);
			expect(result.query).toBe("100");
		...
		test("query params", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/superset-sh/choros/issues/100?q=1",
				repo,
				"issue",
			);
			expect(result.query).toBe("100");
		...
		test("hash fragment (comment anchor)", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/superset-sh/choros/issues/100#issuecomment-12345",
				repo,
				"issue",
			);
			expect(result.query).toBe("100");
		...
		test("www prefix", () => {
			const result = normalizeGitHubQuery(
				"https://www.github.com/superset-sh/choros/issues/200",
				repo,
				"issue",
			);
		...
		test("http (not https)", () => {
			const result = normalizeGitHubQuery(
				"http://github.com/superset-sh/choros/issues/200",
				repo,
				"issue",
			);
			expect(result.query).toBe("200");
	...
	describe("cross-repo mismatch", () => {
		test("different owner", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/other-org/choros/issues/100",
				repo,
				"issue",
			);
			expect(result.query).toBe("");
