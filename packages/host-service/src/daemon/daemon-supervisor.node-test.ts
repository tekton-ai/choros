...
import { DaemonSupervisor, ptyDaemonSocketPath } from "./daemon-supervisor.ts";
...
afterEach(async () => {
	...
	for (const { sup } of supervisorsToCleanup.splice(0)) {
		try {
			await sup.stop();
		} catch {
			// best-effort
...
describe("DaemonSupervisor.ensure (real spawn)", () => {
	...
	test("flags updatePending when running daemon is older than expected", async () => {
		...
		try {
			...
			fs.mkdirSync(ptyDaemonManifestDir(), {
				recursive: true,
				mode: 0o700,
			});
			...
			const adopted = await sup.ensure();
	...
	test("restart() kills the old daemon and spawns a new one", async () => {
		...
		const a = await sup.ensure();
		const aPid = a.pid;

		await sup.restart();
		const after = (sup as unknown as { instance: { pid: number } | null })
			.instance;
	...
	test("auto-respawns after the running daemon dies unexpectedly", async () => {
		...
		let _next = sup.getSocketPath();
		while (Date.now() < deadline) {
			const inst = (sup as unknown as { instance: { pid: number } | null })
				.instance;
	...
	test("detects when an adopted daemon dies externally", async () => {
		...
		while (Date.now() < deadline) {
			const inst = (supB as unknown as { instance: { pid: number } | null })
				.instance;
			if (!inst) break;
			await new Promise((r) => setTimeout(r, 200));
		}
		const after = (supB as unknown as { instance: { pid: number } | null })
			.instance;
		assert.equal(
			after,
			null,
...
describe("DaemonSupervisor.update (Phase 2 fd-handoff)", () => {
	test("update() preserves live sessions across a daemon binary swap", async () => {
		...
		const { DaemonClient } = await import("../terminal/daemon-client/index.ts");
		...
		const successorInst = (
			sup as unknown as {
				instance: { pid: number; socketPath: string } | null;
			}
		).instance;
	...
	test("auto-update attempts non-destructive handoff with live sessions", async () => {
		// The daemon handoff preserves live file descriptors. A failed background
		// update must leave the predecessor and its sessions untouched.
		...
		try {
			...
			const { DaemonClient } = await import(
				"../terminal/daemon-client/index.ts"
			);
			...
			assert.equal(runUpdateCalled, true);
...
describe("trustd-degraded daemon heal", () => {
	...
	function seedManifest(_orgId: string, pid: number, socketPath: string): void {
		fs.mkdirSync(ptyDaemonManifestDir(), { recursive: true, mode: 0o700 });
		const manifest: PtyDaemonManifest = {
			pid,
			socketPath,
	...
	test(
		...
		async () => {
			...
			supervisorsToCleanup.push({ sup, orgId });
			const instance = await sup.ensure();
	...
	test(
		...
		async () => {
			...
			const adopted = await degradedBoot.ensure();
			...
			const healed = await healthyLaunch.ensure();
...
function dropSupervisorInstance(
	...
	(sup as unknown as { instance: unknown | null }).instance = null;
}
...
describe("manifest-less adoption of a slow daemon (GH #6822)", () => {
	...
	test("adopts a live daemon that answers hello slower than one attempt", async () => {
		...
		try {
			...
			assert.equal(
				fs.existsSync(
					path.join(ptyDaemonManifestDir(), "pty-daemon-manifest.json"),
				),
				false,
