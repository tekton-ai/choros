	...
	test("recovers adoption from a live expected socket when the manifest is missing", async () => {
		...
		try {
			...
			try {
				...
				expect(readPtyDaemonManifest()).toMatchObject({
					pid: process.pid,
					socketPath,
				});
				expect(
...
describe("DaemonSupervisor daemon health", () => {
	...
	test("starts the clock when a live daemon stops answering", async () => {
		...
		try {
			...
			expect(sup.getHealth()?.reachable).toBe(false);
		} finally {
			await fake.close();
		}
	});
	...
	test("clears the silence and backfills the version once it answers again", async () => {
		...
		try {
			...
			expect(sup.getUpdateStatus()?.running).toBe("0.1.0");
		} finally {
			await fake.close();
		}
	});
	...
	test("kicks off auto-update when the late probe reveals a stale version", async () => {
		...
		try {
			...
			expect(sup.getUpdateStatus()?.pending).toBe(true);
...
describe("DaemonSupervisor.getUpdateStatus", () => {
	...
	test("returns null when no instance is registered", () => {
		expect(sup.getUpdateStatus()).toBeNull();
	});
	...
	test("reflects updatePending=false for fresh-spawned instances", () => {
		...
		expect(sup.getUpdateStatus()).toEqual({
			pending: false,
			running: "0.1.0",
			expected: "0.1.0",
	...
	test("'unknown' running version surfaces but is never pending", () => {
		...
		const status = sup.getUpdateStatus();
...
describe("update-pending event debounce", () => {
	...
	test("debounces the singleton version pair", () => {
		const stale = staleInstance("0.0.9");
		invokeMaybeFire(sup, "ignored", stale);
		invokeMaybeFire(sup, "ignored", stale);
		expect(
			loggedEvents.filter(
				(event) => event.event === "pty_daemon_update_pending",
			),
		).toHaveLength(1);
	});
});
...
describe("DaemonSupervisor.restart", () => {
	...
	test("clears the crash circuit so the user can recover from a tripped breaker", async () => {
		(sup as unknown as { circuitOpen: boolean }).circuitOpen = true;
		(sup as unknown as { crashTimes: number[] }).crashTimes = [1, 2, 3, 4];

		await sup.restart();

		expect(sup.isCircuitOpen()).toBe(false);
		expect((sup as unknown as { crashTimes: number[] }).crashTimes).toEqual([]);
	...
	test("awaits an in-flight pendingStart before stopping", async () => {
		...
		(sup as unknown as { pendingStart: Promise<unknown> | null }).pendingStart =
			pendingPromise;

		const stopMock = (sup as unknown as { stop: ReturnType<typeof mock> }).stop;
		const restartPromise = sup.restart();
	...
	test("falls through cleanly if the pendingStart rejects", async () => {
		...
		await expect(sup.restart()).resolves.toEqual({
			success: true,
		});
	});
...
describe("DaemonSupervisor.update concurrency guard", () => {
	...
	test("coalesces updates for the singleton daemon", async () => {
...
describe("DaemonSupervisor.update failure mode", () => {
	...
	test("runUpdate throwing leaves the predecessor instance untouched", async () => {
		...
		await expect(sup.update()).rejects.toThrow(/ECONNRESET/);
...
describe("DaemonSupervisor auto-update best effort", () => {
	...
	test("does not overwrite the current daemon if the failed update changed it", async () => {
		...
		expect(sup.getUpdateStatus()?.autoUpdateFailure).toBeNull();
		expect(
			loggedEvents.some(
				(e) =>
					e.event === "pty_daemon_auto_update_failed" &&
...
function seedPredecessor(
	...
	(sup as unknown as { instance: unknown }).instance = {
		pid,
...
function getInstancePid(
	...
	return (sup as unknown as { instance: { pid: number } | null }).instance?.pid;
}
...
function seedDaemonInstance(
	...
	(sup as unknown as { instance: unknown }).instance = instance;
}
...
function invokeMaybeFire(
	...
	(
		sup as unknown as {
			maybeFireUpdatePending: (value: ReturnType<typeof staleInstance>) => void;
		}
	).maybeFireUpdatePending(instance);
}
...
function invokeKickoffAutoUpdate(
	...
	(
		sup as unknown as {
			kickoffAutoUpdate: (value: ReturnType<typeof staleInstance>) => void;
		}
	).kickoffAutoUpdate(instance);
}
...
function invokeRefreshReachability(
	...
	return (
		sup as unknown as {
			refreshReachability: (pid: number) => Promise<void>;
		}
	).refreshReachability(pid);
}
...
describe("ptyDaemonSocketPath", () => {
	const defaultPath = () => {
		const home = path.resolve(path.join(os.homedir(), ".choros"));
	...
	test("default homes use the stable machine-local path", () => {
		expect(ptyDaemonSocketPath({ NODE_ENV: "production" })).toBe(defaultPath());
		expect(
			ptyDaemonSocketPath({
				NODE_ENV: "production",
	...
	test("equivalent spellings of one custom home share a socket", () => {
		const canonical = ptyDaemonSocketPath({
			NODE_ENV: "production",
	...
	test("any non-default home is namespaced, regardless of NODE_ENV", () => {
		...
		for (const NODE_ENV of ["production", "test", undefined]) {
			...
			).not.toBe(defaultPath());
		}
	});

	test("test-runner contexts require an isolated home", () => {
		expect(() => ptyDaemonSocketPath({ NODE_ENV: "test" })).toThrow(
			/isolated temp dir/,
		);
		expect(() =>
			ptyDaemonSocketPath({
	...
	test("non-default development homes get their own stable daemon socket", () => {
		const a = ptyDaemonSocketPath({
			NODE_ENV: "development",
			CHOROS_HOME_DIR: "/tmp/home-a",
		});
		const b = ptyDaemonSocketPath({
			NODE_ENV: "development",
			CHOROS_HOME_DIR: "/tmp/home-b",
		});
		expect(a).not.toBe(defaultPath());
		expect(b).not.toBe(defaultPath());
		expect(a).not.toBe(b);
		expect(
			ptyDaemonSocketPath({
				NODE_ENV: "development",
	...
	test("development with a default home uses the same singleton path", () => {
		expect(ptyDaemonSocketPath({ NODE_ENV: "development" })).toBe(
			defaultPath(),
		);
		expect(
			ptyDaemonSocketPath({
	...
	test("stays under Darwin's 104-byte sun_path limit for long worktree homes", () => {
		const socket = ptyDaemonSocketPath({
			NODE_ENV: "development",
			CHOROS_HOME_DIR:
