...
import { DaemonClient } from "../terminal/daemon-client/index.ts";
...
/** Stable, short socket path scoped only by this Choros home directory. */
export function ptyDaemonSocketPath(
	env: NodeJS.ProcessEnv = process.env,
): string {
	assertIsolatedDaemonNamespaceInTests(env);
	const home = path.resolve(
		env.CHOROS_HOME_DIR || path.join(os.homedir(), ".choros"),
	);
	const shortId = createHash("sha256").update(home).digest("hex").slice(0, 12);
...
export class DaemonSupervisor {
	...
	private pendingStart: Promise<DaemonInstance> | null = null;
	private crashTimes: number[] = [];
	private stopping = false;
	private circuitOpen = false;
	private lastUpdatePendingPair: string | null = null;
	private healthPollTimer: ReturnType<typeof setInterval> | null = null;
	private updateInFlight: Promise<
	 ...
	 */
	private async killAdoptedDaemon(instance: DaemonInstance): Promise<void> {
	 ...
	 * (probe failure ≠ stale).
	 */
	getUpdateStatus(): DaemonUpdateStatus | null {
	...
	update(): Promise<
		{ ok: true; successorPid: number } | { ok: false; reason: string }
	> {
		return this.startUpdate().promise;
	}
	...
	private startUpdate(): {
		...
		const promise = this.runUpdate().finally(() => {
			this.updateInFlight = null;
		});
		this.updateInFlight = promise;
		return { promise, started: true };
	}

	private async runUpdate(): Promise<
		{ ok: true; successorPid: number } | { ok: false; reason: string }
	> {
		const instance = this.instance;
		if (!instance) {
			return { ok: false, reason: "no daemon running" };
		}
		...
		const existingManifest = readPtyDaemonManifest();
		if (existingManifest) {
			writePtyDaemonManifest({
				...existingManifest,
		...
		const restoreOnFailure = () => {
			...
			this.startAdoptedLivenessCheck(instance.pid);
		};
		...
		this.instance = successorInstance;
		this.stopping = false;
		this.lastUpdatePendingPair = null;
		...
		this.startAdoptedLivenessCheck(result.successorPid);
	 ...
	 */
	async restart(): Promise<{ success: true }> {
	...
	private async forceRestart(log: {
		event: string;
		props: Record<string, unknown>;
	}): Promise<{ success: true }> {
		const prev = this.instance;
		const hadCircuitOpen = this.circuitOpen;
	 ...
	 */
	async ensure(): Promise<DaemonInstance> {
		if (this.circuitOpen) {
			throw new Error(
				`[pty-daemon] crash circuit open: ${CRASH_BUDGET} crashes within ${CRASH_WINDOW_MS / 1000}s. Restart the host-service to retry.`,
			);
		}
		...
		const startPromise = this.start().finally(() => {
	...
	getSocketPath(): string | null {
		return this.instance?.socketPath ?? null;
	}
	 ...
	 */
	async listSessions(timeoutMs = 1500): Promise<SessionInfo[] | null> {
	 ...
	 */
	private startAdoptedLivenessCheck(pid: number): void {
		this.startHealthPoll(pid, () => {
			console.log(
				`[pty-daemon] adopted process ${pid} died — clearing instance for next-ensure respawn`,
			);
	 ...
	 */
	private startHealthPoll(pid: number, onDeath: (() => void) | null): void {
		this.stopHealthPoll();
		let probing = false;
		let lastProbeAt = 0;
		const timer = setInterval(() => {
			...
			void this.refreshReachability(pid).finally(() => {
				probing = false;
			});
	 ...
	 */
	private async refreshReachability(pid: number): Promise<void> {
		const instance = this.instance;
		...
		if (current.runningVersion === "unknown") {
			...
			this.maybeFireUpdatePending(current);
			if (current.updatePending && this.opts.autoUpdate !== false) {
				this.kickoffAutoUpdate(current);
			}
		}
		...
		if (
			...
			if (probe.trustdHealthy === false && (await this.hostTrustdHealthy())) {
				...
				if (this.instance !== current) return;
				...
				this.stopHealthPoll();
				await this.killAdoptedDaemon(current);
				if (this.instance?.pid === pid) {
					this.instance = null;
				}
				void this.ensure().catch((err) => {
					console.error(
						"[pty-daemon] respawn after late degraded report failed:",
						err,
					);
				});
			}
	...
	private stopHealthPoll(): void {
		const timer = this.healthPollTimer;
		if (timer) {
			clearInterval(timer);
	 ...
	 */
	private kickoffAutoUpdate(instance: DaemonInstance): void {
		logEvent("pty_daemon_auto_update_attempt", {
			runningVersion: instance.runningVersion,
			expectedVersion: instance.expectedVersion,
			pid: instance.pid,
		});
		void this.runAutoUpdate(instance).catch((err) => {
	...
	private async runAutoUpdate(instance: DaemonInstance): Promise<void> {
		const update = this.startUpdate();
		try {
			...
			if (leftPending) {
				this.recordAutoUpdateFailure(instance, result.reason);
			}
		} catch (err) {
			...
			if (leftPending) {
				this.recordAutoUpdateFailure(instance, reason);
			}
		}
	}
	...
	private isAutoUpdateFailureStillPending(
		...
		return (
			updateStartedHere && this.instance === instance && instance.updatePending
		);
	}
	 ...
	 */
	private async killStaleDaemonForDev(): Promise<void> {
		...
		console.log(
			`[pty-daemon] DEV: killing leftover daemon pid=${manifest.pid} (started ${Math.round((Date.now() - manifest.startedAt) / 1000)}s ago) so the next bootstrap picks up fresh bundle code`,
		);
		await terminateProcessTreeAndGroups(manifest.pid, "SIGTERM");
		removePtyDaemonManifest();
	}

	private async start(): Promise<DaemonInstance> {
		...
		if (shouldKillStaleDaemonForDev()) {
			await this.killStaleDaemonForDev();
		}

		let adopted = await this.tryAdopt();
		...
		if (
			...
			await this.killAdoptedDaemon(adopted);
			adopted = null;
		}
		if (adopted) {
			this.instance = adopted;
			console.log(
				`[pty-daemon] adopted existing daemon pid=${adopted.pid} runningVersion=${adopted.runningVersion} updatePending=${adopted.updatePending}`,
			);
			...
			this.maybeFireUpdatePending(adopted);
			this.startAdoptedLivenessCheck(adopted.pid);
			...
			if (adopted.updatePending && this.opts.autoUpdate !== false) {
				this.kickoffAutoUpdate(adopted);
			}
			return adopted;
		}
	 ...
	 */
	private maybeFireUpdatePending(instance: DaemonInstance): void {
		if (!instance.updatePending) {
			this.lastUpdatePendingPair = null;
			return;
		}
		const pair = `${instance.runningVersion}:${instance.expectedVersion}`;
		if (this.lastUpdatePendingPair === pair) return;
		this.lastUpdatePendingPair = pair;
	...
	private async tryAdopt(): Promise<DaemonInstance | null> {
		const manifest = readPtyDaemonManifest();
		const expectedSocketPath = ptyDaemonSocketPath();
		if (!manifest) {
			return this.tryAdoptFromSocket(expectedSocketPath, {
		...
		if (
			...
			return (
				...
				this.tryAdoptFromSocket(expectedSocketPath, {
	...
	private async tryAdoptFromSocket(
		...
		return this.adoptFromProbe(socketPath, probe, context.reason);
	}
	...
	private async spawn(): Promise<DaemonInstance> {
		...
		const socketPath = ptyDaemonSocketPath();
		...
		if (!fs.existsSync(this.opts.scriptPath)) {
			throw new Error(
				`[pty-daemon] script not found at ${this.opts.scriptPath} — has the daemon binary been bundled?`,
			);
		}
		...
		console.log(
			`[pty-daemon] spawning ${this.opts.scriptPath} → ${socketPath} (log: ${logPath})`,
		);
		...
		if (!childPid) {
			throw new Error("[pty-daemon] failed to spawn");
		}
		...
		if (!ready) {
			...
			throw new Error(
				`[pty-daemon] socket did not become ready within ${SOCKET_READY_TIMEOUT_MS}ms (childPid=${childPid}, earlyExit=${earlyExitCode ?? earlyExitSignal ?? "still alive"}). Log tail:\n${logTail}`,
			);
		}

		if (!isDev) child.unref();
		child.on("exit", (code) => {
			console.log(`[pty-daemon] exited with code ${code}`);
			const current = this.instance;
			...
			const now = Date.now();
			const recent = (this.crashTimes ?? []).filter(
				(t) => now - t < CRASH_WINDOW_MS,
			);
			recent.push(now);
			this.crashTimes = recent;
			...
			if (recent.length > CRASH_BUDGET) {
				...
				console.error(
					`[pty-daemon] crash circuit OPEN — ${recent.length} crashes in ${CRASH_WINDOW_MS / 1000}s; refusing further respawns until clearCrashCircuit() is called`,
				);
			...
			console.warn(
				`[pty-daemon] auto-respawning after unexpected exit (${recent.length}/${CRASH_BUDGET} in window)`,
			);
			void this.ensure().catch((err) => {
				console.error("[pty-daemon] auto-respawn failed:", err);
			});
		});
		...
		this.startHealthPoll(childPid, null);
		console.log(`[pty-daemon] spawned pid=${childPid} socket=${socketPath}`);
		return instance;
	}
}
