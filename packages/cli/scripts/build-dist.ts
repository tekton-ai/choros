/**
 * Builds a standalone Choros CLI distribution tarball.
 *
 * Bundle layout (extracts into ~/choros/):
 *   bin/choros                 — Bun-compiled CLI binary
 *   bin/choros-host            — Shell wrapper to run the host-service
...
async function downloadAndExtractNode(
	...
	const cacheDir = join(homedir(), ".choros-build-cache");
...
`;
	const wrapperPath = join(binDir, "choros-host");
...
async function main(): Promise<void> {
	...
	const cliDir = resolve(import.meta.dir, "..");
	const stagingRoot = join(cliDir, "dist", `choros-${target}`);
	...
	await buildCli(target, join(stagingRoot, "bin", "choros"));
	...
	cpSync(
		resolve(import.meta.dir, "../../../plugins/choros"),
	...
	const tarball = join(cliDir, "dist", `choros-${target}.tar.gz`);
