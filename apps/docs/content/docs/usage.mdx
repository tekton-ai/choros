...
Quota comes from the providers' own usage endpoints using your existing sign-ins; Choros never handles credentials. Some plans (org-managed, education) don't expose limits.
...
Under the hood, a profile is just a config dir the CLI is pointed at (`CLAUDE_CONFIG_DIR` / `CODEX_HOME`). Choros's agent wrappers re-resolve the current default every time the agent starts in a Choros terminal — so switching works for agents you launch by hand too, and a value you export yourself always wins.
...
The **Machine resources** section is a live view (every 2 seconds) of local CPU and memory: Choros's own usage, the system RAM split, and a process table covering the app and every workspace, grouped by project down to individual terminal panes. Click a workspace or session row to jump to it.
