...
// reads CHOROS_API_KEY + CHOROS_ORGANIZATION_ID from env

const TARGET_HOST_ID = process.env.CHOROS_TARGET_HOST_ID!;  // see hosts.list()
const PROJECT_ID = process.env.CHOROS_PROJECT_ID!;          // see projects.list()
...
app.post('/webhooks/sentry', async (c) => {
  ...
  return c.json({
    ...
    sessionId: result.agents[0]?.ok ? result.agents[0].sessionId : null,
  });
});
...
const client = new Choros();
const HOST_ID = process.env.CHOROS_HOST_ID!;
const PROJECT_ID = process.env.CHOROS_PROJECT_ID!; // see projects.list()
...
const projects = await client.projects.list({ hostId: HOST_ID });
...
const totalAgents = wave.reduce(
  (n, result) => n + result.agents.filter((agent) => agent.ok).length,
  0,
);
console.log(`launched ${totalAgents} agents across ${projects.length} repos`);
```
...
console.log(`dispatched run ${run.runId} for automation ${run.automationId}`);
```
