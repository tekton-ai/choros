...
description: Drive Choros programmatically from any TypeScript or JavaScript codebase.
---

<Callout type="warn">
The SDK is in early alpha. It is not meant for production use and may be removed in the future. Stay on the latest version (`@choros_sh/sdk@latest`) while we iterate.
</Callout>

The Choros TypeScript SDK lets you create and manage your workspaces, tasks, projects, and automations from code. Anything you can do in the dashboard, your scripts and services can do too.
...
<Tab value="pnpm">
```bash
pnpm add @choros_sh/sdk
```
</Tab>
<Tab value="yarn">
```bash
yarn add @choros_sh/sdk
```
</Tab>
<Tab value="bun">
```bash
bun add @choros_sh/sdk
```
</Tab>
</Tabs>
...
1. Open the [Choros desktop app](https://choros.sh/download) → **Settings → API Keys** → **Create**. The key starts with `sk_live_…`. Copy it now (you won't be able to see it again).
2. Note your **organization ID**. Find it in the org switcher inside the desktop app.

```bash
export CHOROS_API_KEY=sk_live_…
export CHOROS_ORGANIZATION_ID=…
```
...
// reads CHOROS_API_KEY and CHOROS_ORGANIZATION_ID from env
```
...
```ts
import Choros, { APIError, NotFoundError, RateLimitError } from '@choros_sh/sdk';
...
try {
  ...
  if (err instanceof RateLimitError) {
    ...
    console.error(`Choros returned ${err.status}:`, err.error);
  }
}
```
