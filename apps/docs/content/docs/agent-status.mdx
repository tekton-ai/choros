...
description: How Choros knows what your agents are doing, and how it tells you
---
...
Choros installs lifecycle hooks and command wrappers into each agent's config (the entries in `~/.superset/bin` and files like `choros-hooks`). Those hooks report when an agent starts, finishes, or is waiting for input. That's why status only works for agents launched through Choros, and why removing the hooks disables status and notifications.

The hooks no-op outside Choros terminals (they check for a Choros-supplied env var). To remove them from an agent's config entirely, turn off the agent's **Choros hooks** toggle in **Settings → Agents**.
...
macOS only shows dock badges when Choros has notification permission. If you don't see badges, check **System Settings → Notifications → Choros**.
