...
Choros installs lifecycle hooks and command wrappers into each agent's config (the entries in `~/.choros/bin` and files like `choros-hooks`). Those hooks report when an agent starts, finishes, or is waiting for input. That's why status only works for agents launched through Choros, and why removing the hooks disables status and notifications.
