...
your Choros settings and shown in the Scripts bar.

Create `.choros/config.json` in your project:
...
|----------|-------------|
| `CHOROS_ROOT_PATH` | Path to root repository |
...
```
~/.choros/projects/<repo-path>/config.json
```

For a repo at `/Users/me/work/app`, that's `~/.choros/projects/Users/me/work/app/config.json`. The legacy location keyed by project ID (`~/.choros/projects/<project-id>/config.json`) still works as a fallback.

### Priority Order

For each key (`setup`, `teardown`, `run`), the highest-priority source that defines it wins:

1. `~/.choros/projects/<repo-path>/config.json`: user override
2. `<worktree>/.choros/config.json`: workspace-specific (setup and teardown only)
3. `<repo>/.choros/config.json`: project default
...
If no commands are configured for a key, Choros looks for a script instead: `.choros/<key>.sh` (`setup.sh`, `teardown.sh`, `run.sh`) in the worktree first, then the main repo. A gitignored script isn't checked out into new worktrees, so it typically exists only in the main repo.
...
```json
{
  "setup": ["~/.choros/projects/<repo-path>/setup.sh"],
  "teardown": ["~/.choros/projects/<repo-path>/teardown.sh"]
}
```
...
Extend or override your team's committed scripts without modifying the repo. Create `.choros/config.local.json` alongside `config.json`. It's gitignored automatically.
...
```json
{
  "setup": {
    ...
    "after": ["./.choros/my-post-setup.sh"]
  },
  "teardown": {
    "after": ["./.choros/my-cleanup.sh"]
  }
}
```
...
```json
{
  "setup": ["./.choros/my-custom-setup.sh"]
}
```
...
- Commit `.choros/` to share with team
- Use shell scripts for complex logic: `"setup": ["./.choros/setup.sh"]`
- Use `config.local.json` to add personal steps without touching the team's config
- Use user overrides (`~/.choros/projects/`) to replace scripts entirely per-project
