...
description: Open Linear issues as Choros workspaces with a custom script
---
...
**Work on issue → Custom script** creates a Choros workspace for the issue
...
1. `choros workspaces create`: creates a local workspace on the issue's
   branch and launches an agent with the issue as its prompt.
2. `choros workspaces open`: opens that workspace in the desktop app via its
   deep link.

## Prerequisites

- The [Choros CLI](/cli/getting-started) installed and signed in
  (`choros auth login`).
- The Choros desktop app installed (the script opens workspaces in it).
...
Save this as `~/.linear/open-in-choros.sh` and replace `PROJECT` with the ID
from above:

```bash
#!/usr/bin/env bash
# Linear custom script -> open the issue as a Choros workspace.
set -euo pipefail
...
choros start --daemon >/dev/null 2>&1 || true
...
choros workspaces open "$id"
```

Make it executable:

```bash
chmod +x ~/.linear/open-in-choros.sh
```

<Callout type="info">
The script depends on `choros` being on your `PATH` and `jq` being installed.
If `choros` isn't on your `PATH`, use its full path (the desktop app installs
a shim at `~/.superset/bin/choros`).
</Callout>

The branch named by the issue won't exist yet. `workspaces create` forks it
from the project's default branch automatically. `--skip-branch-prefix` keeps
Linear's exact branch format (so Linear autolinks the branch back to the
issue) even when the project has a branch prefix configured. Re-running on
the same issue reuses the existing workspace instead of creating a duplicate.

<Callout type="info">
With the Linear integration connected, Choros does this natively: linking
an issue in the New Workspace flow (or opening a task in a workspace) uses
Linear's branch name automatically and moves the issue to In Progress. The
script above is for driving it from Linear's side.
</Callout>
...
{
  "openIssue": {
    "path": "/Users/you/.linear/open-in-choros.sh",
    "env": [
...
Linear runs the script, and the workspace opens in Choros with the agent
...
- **Multiple repos.** The project ID is fixed in the script. To route different
  Linear teams or projects to different Choros projects, branch on a
