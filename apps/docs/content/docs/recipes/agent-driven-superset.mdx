---
title: Drive Choros from Your Agent
...
<Callout type="info" title="Use when">
	you're mid-conversation with an agent and the next step is a Choros action (fan this work out in parallel, or make it recur on a schedule) and you'd rather say so than click through the app.
</Callout>

## The Idea

Choros ships [skills](/skills) to your coding agents automatically, so the agent in front of you already knows how to run Choros itself. `choros:orchestrate` makes it a coordinator of parallel workers; that has [its own page](/orchestration). This recipe is about the move that follows: `choros:automate` turns anything you've asked for twice into a scheduled [automation](/automations), in the same conversation.
...
```text
That worked. Make it a Choros automation that runs every Monday at 7am
...
- **When Choros itself misbehaves**: "diagnose why this host shows offline" has the agent fix it instead of you reading logs
