...
Every terminal Choros runs is addressable from the [CLI](/cli/getting-started): `terminals list`, `read`, and `send`. Compose them with `workspaces list` and one shell loop sweeps a whole project: every agent's screen in one scroll, or one instruction delivered to all of them.
...
```bash
choros workspaces list --project my-app --quiet | while read ws; do
  choros terminals list --workspace "$ws" --json 2>/dev/null \
    | jq -r '.sessions[].terminalId' | while read t; do
      echo "=== $ws / $t ==="
      choros terminals read --workspace "$ws" --terminal "$t" --max-lines 5 --json | jq -r '.text'
  done
done
```
...
choros workspaces list --project my-app --quiet | while read ws; do
  choros terminals list --workspace "$ws" --json 2>/dev/null \
    | jq -r '.sessions[].terminalId' | while read t; do
      choros terminals send --workspace "$ws" --terminal "$t" \
...
```bash
choros workspaces list --project my-app --quiet | xargs -L1 choros workspaces delete
```
...
- **Let an agent do this**: the [`choros:orchestrate` skill](/orchestration) runs these same loops for you; monitoring workers and delivering follow-ups is exactly how it coordinates
