...
description: Install the Choros CLI, sign in, and run your first commands.
---
...
The Choros CLI (`choros`) is a single static binary that drives the same
...
The desktop app includes the Choros CLI. When the app launches, it writes
an app-managed shim at `~/.choros/bin/choros`, and Choros terminals
prepend `~/.choros/bin` to `PATH` automatically.
...
x64 or arm64), installs Choros under `~/choros` by default, and adds
`~/choros/bin` to your shell `PATH`.

### Homebrew

```bash
brew install choros-sh/tap/choros
```
...
consent page, and stores a session token at `~/.choros/config.json`.
...
…or set `CHOROS_API_KEY` per-invocation without writing anything to disk:

```bash
export CHOROS_API_KEY=sk_live_…
choros auth whoami
```
...
Bare `choros` in a terminal opens an interactive command browser:
...
command before running it. `choros --help` prints the same catalog as
static, grouped help. Agents and CI always get static help; the browser
never opens there.

![The interactive command browser](/images/cli-command-browser.png)
...
```bash
# Clone from a URL
choros projects create --name "my-app" --local \
...
```bash
choros projects setup <id-from-projects-list> --local --parent-dir ~/code
```
...
```bash
choros tasks list --status in_progress --json | jq '.[].title'
choros workspaces list --quiet | xargs -L1 choros workspaces delete
```

When the CLI detects an agent or CI environment (`CLAUDE_CODE`,
`CLAUDECODE`, `CLAUDE_CODE_ENTRYPOINT`, `CODEX_CLI`, `GEMINI_CLI`,
`CHOROS_AGENT`, or `CI` set to a non-empty value), it defaults to JSON
...
```bash
choros hosts list
choros workspaces list              # this machine
choros workspaces list --host h_…  # another host
```
...
```text
~/.choros/config.json                            # auth, active organization
~/.choros/host/<organizationId>/manifest.json    # host server endpoint + token
~/.choros/host/<organizationId>/host.db          # local host server DB
```

`CHOROS_HOME_DIR` relocates the whole tree, matching the desktop app.
