...
| `SUPERSET_HOME_DIR` | Directory the CLI uses for `config.json` and the host server tree. Default: `~/.choros`. Shared with the desktop app. |
...
| Variable | Description |
| --- | --- |
| `SUPERSET_HOME` | Install root for the standalone CLI and host binary. Default: `~/choros`. |
