...
The `choros start` command runs a headless HTTP server that any Choros desktop app or CLI can communicate with.
...
`~/.choros/bin/choros` shim on app launch. Choros terminals prepend
...
## Authenticate the Choros CLI

The host server starts under your Choros session. If you haven't already:
...
`~/.choros/host/<organizationId>/manifest.json` so other clients can find
...
A workspace branches off a project: a repo registered with Choros.
...
choros projects setup 47c31b04-… --local --import ~/code/repo
```
...
State lives under `~/.choros/host/<organizationId>/`:

```text
~/.choros/host/<organizationId>/manifest.json    # endpoint + auth token
...
vice-versa. `CHOROS_HOME_DIR` relocates the whole tree.

## Command details

For full flags, exit codes, and output shapes:

- [`choros start`](/cli/cli-reference#choros-start)
- [`choros status`](/cli/cli-reference#choros-status)
- [`choros stop`](/cli/cli-reference#choros-stop)
