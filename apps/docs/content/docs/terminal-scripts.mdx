...
<YouTubeVideo id="jT8AoqYTwRU" title="Choros terminal scripts" thumbnail="/images/preset.png" />
...
| --- | --- | --- |
| **Terminal script** | A reusable command or group of commands you launch from the app | Choros user settings |
| **Project lifecycle script** | Repository setup, teardown, or Run-button behavior | `.choros/config.json` or `.choros/*.sh` |

Older versions of Choros called terminal scripts **presets**. You may still
...
`choros presets add` remains an alias for compatibility. See the
...
The bar includes a built-in **Choros CLI** script that runs `choros --help`.
...
`autoApplyDefaultPreset`, visible in `choros settings list`) also gate
auto-run behavior.

If more than one Run source matches a workspace, Choros uses the first
