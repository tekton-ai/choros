---
title: Choros MCP Server
description: Connect AI agents directly to Choros via MCP
---

<Callout type="info">
The Choros MCP server lives at `https://api.choros.sh/mcp`. The versioned URL (`/api/v2/agent/mcp`) keeps working for existing configurations; the legacy v1 server (`/api/agent/mcp`) has been removed and returns `410 Gone`.
</Callout>

## Overview

Choros provides an [MCP (Model Context Protocol)](https://modelcontextprotocol.io) server that lets AI agents interact with your tasks, workspaces, and devices programmatically.
...
```bash title="terminal"
claude mcp add choros --transport http https://api.choros.sh/mcp
```
</Tab>
<Tab value="Codex">
```bash title="terminal"
codex mcp add choros --url https://api.choros.sh/mcp
```
</Tab>
<Tab value="Gemini CLI">
```bash title="terminal"
gemini mcp add --transport http choros https://api.choros.sh/mcp
```
</Tab>
<Tab value="Open Code">
...
[2m│[0m  [36mhttps://api.choros.sh/mcp[0m
[2m│[0m
...
<Tab value="Amp CLI">
```bash title="terminal"
amp mcp add --workspace choros https://api.choros.sh/mcp
```
</Tab>
...
name = "choros"
transport = "http"
url = "https://api.choros.sh/mcp"
```
</Tab>
<Tab value="Kimi Code">
Run `kimi`, enter `/mcp-config`, and add an HTTP server named `choros` with this URL:

```text title="Kimi Code"
https://api.choros.sh/mcp
```

Then enter `/mcp-config login choros` to authorize it.
</Tab>
<Tab value="Grok">
Run `grok`, enter `/mcps`, and add an HTTP server named `choros` with this URL:
...
```json title="settings.json"
{
  "mcpServers": {
    "choros": {
      "httpUrl": "https://api.choros.sh/mcp"
    }
  }
}
```
...
Then enter `/mcp-config login choros` in Kimi Code to authorize it.
</Tab>
<Tab value="Grok">
