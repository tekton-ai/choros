...
Choros includes a built-in chat panel for AI interactions without leaving the app.
...
- **Model Override** - Optional model override for Choros Chat
...
- **On** - Terminal agents run their command right away, and Choros Chat sends the task prompt immediately
- **Off** - Terminal agents stage the command without pressing Enter, and Choros Chat opens as a draft instead

The setting is remembered between sessions.

## Resume Interrupted Sessions

If an agent's terminal dies under it (a crash, a reboot, a killed process), Choros resumes the session automatically: the pane relaunches the agent with its resume command (e.g. `claude --resume <session-id>`) and shows a "Resuming…" pill while it happens. Only unexpected deaths qualify: sessions you close or kill deliberately, and agents that exited cleanly, stay down.
...
```bash
choros agents create --workspace ws_… --agent claude --resume-session <sessionId>
```
...
Teach any agent to drive Choros itself with the official [skills](https://github.com/choros-sh/skills), one for the `choros` CLI and one for the [MCP server](/mcp-server):
