...
Choros includes a `computer` skill that teaches agents to operate native
...
<Callout type="info" title="Optional third-party software">
	Drivers are not built or maintained by Choros — Cua Driver comes from
	[Cua](https://cua.ai) and Peekaboo from [its open-source
	project](https://peekaboo.sh) — and nothing in Choros requires one:
...
The skill is bundled with Choros, but the driver runtime is installed
...
Ask naturally, or invoke `/choros:computer` in Claude Code and
`/choros/computer` in Choros's in-app chat:
...
Use [`browser`](/browser) for a page open in Choros's in-app browser pane. It
provides direct DOM, console, screenshot, and Chrome DevTools Protocol access.

Use `computer` for native applications, the Choros desktop UI itself, browser
