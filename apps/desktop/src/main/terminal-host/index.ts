 ...
 * - Socket: ~/.choros/terminal-host.sock
 * - Auth token: ~/.choros/terminal-host.token
 */
...
// CHOROS_DIR_NAME is imported from shared/constants for multi-worktree support
// This allows workspace-specific home directories (e.g., ~/.choros-my-feature)
...
async function startServer(): Promise<void> {
	// Ensure choros directory exists with proper permissions
	if (!existsSync(CHOROS_HOME_DIR)) {
		mkdirSync(CHOROS_HOME_DIR, { recursive: true, mode: 0o700 });
		log("info", `Created directory: ${CHOROS_HOME_DIR}`);
	}
...
async function main() {
	...
	log("info", `Home directory: ${CHOROS_HOME_DIR}`);
