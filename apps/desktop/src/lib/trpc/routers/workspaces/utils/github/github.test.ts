...
describe("getPullRequestRepoArgs", () => {
	test("returns upstream repo args for forks", () => {
		expect(
			getPullRequestRepoArgs({
				isFork: true,
				upstreamUrl: "git@github.com:choros-sh/choros.git",
			}),
		).toEqual(["--repo", "choros-sh/choros"]);
	});
	...
	test("returns no repo args for non-forks", () => {
		expect(
			getPullRequestRepoArgs({
				isFork: false,
				upstreamUrl: "https://github.com/choros-sh/choros",
			}),
		).toEqual([]);
	});
...
describe("shouldRefreshCachedRepoContext", () => {
	test("returns false when no cached repo context exists", () => {
		expect(
			shouldRefreshCachedRepoContext({
				originUrl: "https://github.com/choros-sh/choros",
	...
	test("returns false when the cached repo still matches origin", () => {
		expect(
			shouldRefreshCachedRepoContext({
				originUrl: "https://github.com/choros-sh/choros",
				cachedRepoContext: {
					repoUrl: "https://github.com/choros-sh/choros",
					upstreamUrl: "https://github.com/choros-sh/choros",
					isFork: false,
				},
			}),
						...
						nodes: [
							{
								...
								url: "https://github.com/choros-sh/choros/pull/2681#discussion_r42",
			...
			parseConversationCommentsResponse([
				{
					...
					html_url:
						"https://github.com/choros-sh/choros/pull/2681#issuecomment-7",
				},
			]),
		).toEqual([
			{
				...
				url: "https://github.com/choros-sh/choros/pull/2681#issuecomment-7",
				kind: "conversation",
				isResolved: false,
			},
		]);
