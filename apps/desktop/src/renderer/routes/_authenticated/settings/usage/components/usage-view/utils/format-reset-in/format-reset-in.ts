...
export function formatResetIn(resetsAt: Date, now: Date = new Date()): string {
	...
	if (days > 0) {
		return hours > 0
			? i18n._(
					msg({
						id: "settings.usage.resetIn.daysHours",
						message: `${days}d ${hours}h`,
					}),
				)
			: i18n._(msg({ id: "settings.usage.resetIn.days", message: `${days}d` }));
	}
	if (hours > 0) {
		return minutes > 0
			? i18n._(
					msg({
						id: "settings.usage.resetIn.hoursMinutes",
						message: `${hours}h ${minutes}m`,
					}),
				)
			: i18n._(
					msg({ id: "settings.usage.resetIn.hours", message: `${hours}h` }),
				);
	}
	return i18n._(
		msg({ id: "settings.usage.resetIn.minutes", message: `${minutes}m` }),
	);
}
...
export function formatResetLabel(
	...
	if (diffMs <= 0) {
		return i18n._(
			msg({ id: "settings.usage.resetLabel.now", message: "Resets now" }),
		);
	}
	...
	return i18n._(
		msg({
			id: "settings.usage.resetLabel.resetsIn",
