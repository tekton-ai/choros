...
export interface UseFolderFirstImportResult {
	start: () => Promise<FinalizedProjectSetupResult | null>;
}
...
export function useFolderFirstImport(options?: {
	...
	const start =
		useCallback(async (): Promise<FinalizedProjectSetupResult | null> => {
			...
			try {
				...
				if ("needsGitInit" in response && response.needsGitInit) {
					...
					return finalizeSetup(activeHostUrl, result, profileContext);
				}
