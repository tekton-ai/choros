...
export function useDashboardSidebarData() {
	...
	const hostsByMachineId = useMemo(
		() =>
			new Map(
				machineId
					? [[machineId, { isOnline: activeHostUrl !== null }] as const]
					: [],
			),
	...
	const pullRequestQueryTargets = useMemo<PullRequestQueryTarget[]>(() => {
		if (!machineId) return [];
		const workspaceIds = visibleSidebarWorkspaces
			.filter((workspace) => workspace.projectId !== null)
	...
	}, [activeHostUrl, machineId, visibleSidebarWorkspaces]);
	...
	const computedGroups = useMemo<DashboardSidebarProject[]>(
		...
		[pullRequestsByWorkspaceId, sidebarProjects, sidebarSections, projectRows],
	);
