...
const WorkspaceHostReadyContext = createContext(false);
export function useWorkspaceHostReady() {
	return useContext(WorkspaceHostReadyContext);
}
...
export function WorkspaceHostGate({
	...
	const { isReachable, isUnreachable, isReconnecting, detail, retry } =
	...
	useEffect(() => {
		if (isUnreachable) reportWorkspaceUnavailable(workspace.id);
	}, [isUnreachable, workspace.id, reportWorkspaceUnavailable]);

	return (
		<WorkspaceHostReadyContext.Provider
			value={isReachable && !isLocalRestartInFlight}
		>
