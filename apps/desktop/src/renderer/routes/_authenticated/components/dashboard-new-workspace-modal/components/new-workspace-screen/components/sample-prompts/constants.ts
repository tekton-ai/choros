...
export const SAMPLE_PROMPTS: Record<string, SamplePrompt> = {
	"set-up-project": {
		...
		prompt: `Set up this repository to work well with Choros workspaces. Read https://tekton-ai.github.io/choros/docs/setup-teardown-scripts and create a .choros/config.json with: setup commands that install dependencies and copy untracked files (like .env) from "$CHOROS_ROOT_PATH" into new workspaces, teardown commands that stop anything setup starts, and a run command that launches the dev server. If parallel workspaces would collide on dev-server ports, make the scripts pick a free port per workspace (see https://tekton-ai.github.io/choros/docs/ports). When you're done, summarize what you configured and how to use it.`,
	},
	"explain-repo": {
		id: "explain-repo",
		label: msg({
	...
	"explain-choros": {
		...
		prompt:
			"Read https://tekton-ai.github.io/choros/docs and figure out how I should be using Choros for this specific repository. Cover how to run several workspaces in parallel without them colliding, what belongs in .choros/config.json, and which agent settings suit this codebase. Be concrete about this repo rather than generic, and end with the two or three changes worth making first.",
	},
};
