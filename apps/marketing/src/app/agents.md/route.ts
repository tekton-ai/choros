...
export async function GET() {
	...
	const lines: string[] = [
		...
		"## Capabilities",
		"",
		"Via the MCP server an authenticated agent can, on behalf of a Choros user:",
		"",
		...
		"- Alternatively, a user-issued Choros API key can be sent as a Bearer token.",
		"",
		"## Setup one-liners",
		"",
		"```bash",
		`claude mcp add choros --transport http ${MCP_SERVER_URL}`,
		`codex mcp add choros --url ${MCP_SERVER_URL}`,
		`gemini mcp add --transport http choros ${MCP_SERVER_URL}`,
		"```",
		"",
		"## Learn more",
		"",
