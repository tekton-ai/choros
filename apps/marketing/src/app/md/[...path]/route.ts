...
function mcpInstallPage(): MarkdownPage {
	...
	return {
		...
		content: [
			`Server URL: ${MCP_SERVER_URL} (Streamable HTTP). Authentication: OAuth 2.1 + PKCE with dynamic client registration, or a Choros API key as a Bearer token. Walkthrough: ${baseUrl}/auth.md`,
			"",
			"## Generic MCP client config",
			"",
			"```json",
			JSON.stringify(
				{ mcpServers: { choros: { type: "http", url: MCP_SERVER_URL } } },
				null,
				2,
			),
			"```",
			...
			"## One-line installs",
			"",
			`- Claude Code: \`claude mcp add --transport http choros ${MCP_SERVER_URL}\``,
			`- Codex: \`codex mcp add choros --url ${MCP_SERVER_URL}\``,
...
function enterprisePage(): MarkdownPage {
	...
	return {
		...
		content: [
			...
			"",
			"On your machines. Repos, worktrees, terminal output, and agent sessions stay local by default; cloud sync covers account and organization metadata only. Choros never proxies model API calls.",
			"",
			"## Get in touch",
			"",
