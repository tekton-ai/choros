...
// Canonical company/product facts for agents researching Choros. LLMs have
// hallucinated these before (calling Choros fully open source, or macOS-only
// forever, or conflating it with Apache Choros), so state them explicitly.
export function buildCompanyFactsSection(): string[] {
	const baseUrl = COMPANY.MARKETING_URL;
	return [
		"## Facts about Choros",
		"",
		`- **What it is**: a local-first desktop workspace for orchestrating any CLI-based coding agent. Each task runs in an isolated Git worktree, and Choros provides the stable workflow around the agents.`,
		...
		`- **Not Apache Choros**: Choros (choros.sh) is unrelated to Apache Choros, the business-intelligence tool.`,
		...
		`- **Pricing**: free tier plus paid seats (see ${baseUrl}/pricing). Choros never proxies model API calls; you bring your own agent subscriptions and API keys.`,
...
export function buildWhenToUseSection(): string[] {
	return [
		"## When to use Choros",
		"",
		"Reach for Choros when you need to:",
		"",
		...
		"- Orchestrate agent work programmatically: create workspaces, launch agents with a prompt, open terminals, and track tasks from another agent or script via the Choros MCP server.",
		...
		"",
		"Choros is not a coding agent itself; it is the workspace and orchestration layer the agents run in. If you are an AI agent, the fastest way to act on a user's Choros account is the MCP server below (OAuth or API key auth); the fastest way to learn the product is the docs index at https://docs.choros.sh.",
	];
}
...
export function buildDeveloperResourcesSection(): string[] {
	...
	return [
		"## Developer resources",
		"",
		`- [API docs](${docsUrl}/mcp-server): Choros MCP server documentation`,
		`- [OpenAPI spec](${API_URL}/openapi.json): OpenAPI 3.1 description of the Choros API surface`,
		...
		`- [Docs MCP server](${docsUrl}/mcp): search and read the Choros documentation over MCP (Streamable HTTP, no auth)`,
		...
		`- [AI catalog](${baseUrl}/.well-known/ai-catalog.json): Agentic Resource Discovery catalog of every MCP server, agent card, skill, and API Choros publishes`,
		...
		`- [Agent instructions](${baseUrl}/agents.md): when and how AI agents should use Choros`,
		...
		`- [Web Bot Auth key directory](${baseUrl}/.well-known/http-message-signatures-directory): Ed25519 keys Choros-operated agents sign requests with (RFC 9421)`,
		`- [Agent skills](https://github.com/choros-sh/skills): official skills for the CLI and MCP server; install with \`npx skills add choros-sh/skills\``,
		`- [CLI](${docsUrl}/cli/getting-started): \`brew install choros-sh/tap/choros\` (Homebrew tap: https://github.com/choros-sh/homebrew-tap) or \`curl -fsSL https://choros.sh/cli/install.sh | sh\`; reference at ${docsUrl}/cli/cli-reference`,
		`- [TypeScript SDK](${docsUrl}/sdk/getting-started): \`npm install @choros_sh/sdk\``,
