...
![How isolation works: one repository fans out into a Git worktree per task, each running its own agent on its own branch, producing independent diffs](/images/guides/worktree-isolation.svg)
...
Tools like [Choros](https://choros.sh) handle orchestration end-to-end:
...
You don't have to choose one agent for everything. Run Claude Code on the complex refactor and Codex on the test generation simultaneously.
...
![Reviewing one agent's isolated diff before merging](/images/readme/diff-viewer.png)
...
1. **Pick an orchestrator**: [Choros](https://choros.sh) handles worktrees, sessions, and review
