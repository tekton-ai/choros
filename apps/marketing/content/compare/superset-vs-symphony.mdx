---
title: "Choros vs Symphony (2026): Agent Workspace vs OpenAI's Tracker-Driven Orchestration"
description: "Compare Choros and OpenAI's Symphony: an interactive workspace for parallel coding agents vs an open-source spec that turns tickets into autonomous Codex runs."
date: 2026-08-07
...
**Verdict:** Symphony is the clearest statement yet of the fully-autonomous thesis -- the ticket board as the only interface -- and its spec is worth reading whatever you run. But it is Codex-only, headless, and a preview by its own label. Choros delivers parallel agent leverage today with the visibility engineers actually want, across any agent, with automations covering the unattended slice. Many teams will eventually want both patterns; Choros is the one you can adopt this afternoon.

---

## Frequently Asked Questions

### Is Symphony a product like Choros?

Not exactly. Symphony is an open-source spec plus a reference implementation that OpenAI calls a "low-key engineering preview" -- teams are encouraged to build their own hardened version. Choros is a shipping desktop product with a CLI, MCP server, and SDK.

### Does Symphony work with Claude Code or other agents?

No. Symphony drives Codex through its app-server protocol exclusively. Choros runs Codex, Claude Code, OpenCode, Cursor, Copilot, Gemini, and any CLI agent.

### Does Symphony use Git worktrees?

No -- it clones the repo into an isolated workspace per issue, contained within a workspace root. Choros uses a Git worktree per task, which shares the repository's object store and is cheaper to create.

### Can Choros do tracker-driven autonomous runs?

Partially. Choros's tasks integrate GitHub Issues and Linear, its automations run scheduled unattended sessions, and its SDK/MCP surfaces support building tracker-driven pipelines -- but it does not ship Symphony's poll-dispatch-land loop as a turnkey daemon. The inverse gap is larger: Symphony has no interactive workspace at all.
